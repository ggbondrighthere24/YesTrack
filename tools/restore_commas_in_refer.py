...
def build_gt_mapping(gt_root: str, seqmap_path: str = None) -> Dict[str, Dict[str, str]]:
    """
    Build mapping per seqid:
      seqid -> { normalized_key (underscore, no-commas) -> target_expr_name (GT, may include commas and hyphens) }
    """
    ...
    for seq_name in gt_seqs:
        ...
        if seqid not in mapping:
            mapping[seqid] = {}
        # If collision occurs, keep the first; we will count ambiguities later.
        mapping[seqid].setdefault(key, expr)
    return mapping
...
def plan_restorations(
    ...
    for seqid, items in refer_items.items():
        ...
        seq_gt_map = gt_map.get(seqid, {})
        # Build reverse map to detect ambiguities (same key -> multiple exprs)
        key_to_exprs: Dict[str, Set[str]] = {}
        for key, expr in seq_gt_map.items():
            key_to_exprs.setdefault(key, set()).add(expr)
