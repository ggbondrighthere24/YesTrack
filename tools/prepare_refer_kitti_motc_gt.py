...
DATASET_SPLITS = {
    "v1": {
        "train": [1, 2, 3, 4, 6, 7, 8, 9, 10, 12, 14, 15, 16, 18, 20],
        "val": [5, 11, 13],
    },
    "v2": {
        "train": [0, 1, 2, 3, 4, 6, 7, 8, 9, 10, 12, 14, 15, 16, 17, 18, 20],
        "val": [5, 11, 13, 19],
    },
}
...
def write_seqmap(seq_names: list, out_seqmap_file: str):
    seqmap_dir = os.path.dirname(out_seqmap_file)
    if seqmap_dir:
        ensure_dir(seqmap_dir)
...
def main():
    ...
    parser.add_argument("--dataset-version", type=str, choices=["v1", "v2"], default="v2", help="Refer-KITTI split definition used to select train/val sequences")
    ...
    split_config = DATASET_SPLITS[args.dataset_version]
    train_names = [f"{i:04d}" for i in split_config["train"] if f"{i:04d}" in all_seqs]
    val_names = [f"{i:04d}" for i in split_config["val"] if f"{i:04d}" in all_seqs]
    ...
    for split, seq_names, seqmap_path in targets:
        ...
        print(f"Prepared {len(out_seq_names)} Refer-KITTI {args.dataset_version} sequences for split '{split}' under {args.out_gt_folder}")
