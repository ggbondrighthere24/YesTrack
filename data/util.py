...
def collate_fn(batch):
    ...
    # Optionally return texts if provided by metas (e.g., ReferKittiMOT)
    batch_texts = []
    has_text = False
    for b in range(len(metas)):
        text = metas[b][0]["text"] if "text" in metas[b][0] else None
        batch_texts.append(text)
        has_text = has_text or (text is not None)
    if has_text:
        res["texts"] = batch_texts
    return res
