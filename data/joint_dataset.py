...
# from .bft import BFT
from .kitti_mot import KittiMOT
from .refer_kitti_mot import ReferKittiMOT
...
class JointDataset(Dataset):
    ...
    def __getitem__(self, info):
        ...
        # For ReferKittiMOT: choose one referring text for the entire clip
        chosen_text = None
        chosen_text_idx = None
        if dataset == "ReferKittiMOT":
            if len(annotations) > 0 and "refer_texts" in annotations[0] and len(annotations[0]["refer_texts"]) > 0:
                chosen_text_idx = random.randint(0, len(annotations[0]["refer_texts"]) - 1)
                chosen_text = annotations[0]["refer_texts"][chosen_text_idx]
                for m in metas:
                    m["text"] = chosen_text
                    m["text_idx"] = chosen_text_idx

        # Apply transforms:
        if self.transforms is not None:
            images, annotations, metas = self.transforms(images, annotations, metas)

        # After transforms, compute is_ref per frame if referring available
        if dataset == "ReferKittiMOT" and chosen_text is not None and chosen_text_idx is not None:
            for t in range(len(annotations)):
                refer_ids_per_text = annotations[t].get("refer_ids_per_text", None)
                referred_ids = [] if refer_ids_per_text is None else refer_ids_per_text[chosen_text_idx]
                referred_ids = set(int(i) for i in referred_ids) if isinstance(referred_ids, list) else set()
                if "id" in annotations[t] and isinstance(annotations[t]["id"], torch.Tensor):
                    curr_ids = annotations[t]["id"].tolist()
                    is_ref = torch.tensor([1.0 if int(_id) in referred_ids else 0.0 for _id in curr_ids], dtype=torch.float32)
                    annotations[t]["is_ref"] = is_ref
                else:
                    annotations[t]["is_ref"] = torch.zeros((annotations[t]["bbox"].shape[0],), dtype=torch.float32)
